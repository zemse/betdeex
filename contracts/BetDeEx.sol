...
contract BetDeEx {
    ...
    function createBet(string memory _description, uint8 _category, uint8 _subCategory, uint256 _minimumBet, uint256 _pricePercent) public onlyManager returns (address) {
        Bet _newBet = new Bet(_description, _category, _subCategory, _minimumBet, _pricePercent);
...
contract Bet {
    ...
    event NewBetting (
        ...
        uint256 _betAmountInExaEs,
        uint8 _choice,
        uint256 _bettorsIndex
    );
    ...
    constructor(string memory _description, uint8 _category, uint8 _subCategory, uint256 _minimumBetInExaEs, uint256 _pricePercentPerThousand) public {
    ...
    function enterBet(uint8 _choice, uint256 _betTokensInExaEs) public {
        ...
        emit NewBetting(msg.sender, _betTokensInExaEs, _choice, _id);
    }

    function endBet(uint8 _choice) public onlyManager {

        // because its required to initialise the pointer with something.
        Bettor[] storage _winnerBettors = drawBettors;
        if (_choice == 0) {
            _winnerBettors = noBettors;
        } else if (_choice == 1) {
